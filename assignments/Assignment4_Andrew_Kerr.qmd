...
## 3. Make an acf and pacf of the data. Suggest a model for the snowshoe hare fur based on
the acf/pacf.
...
The above ACF and plot appears to tail off, while the PACF plot appears to cut off after lag 1. I will try a differencing value of 1 since the above plots do not indicate the data is stationary (I am not sure if I am supposed to check for more since the question asks to only go off of these two plots). With all this in mind, I suggest an ARIMA(1,1,0) model.
...
fit_pelt <- pelt %>%
  as_tsibble() %>%
  model(ARIMA(Hare ~ pdq(1, 1, 0) + PDQ(0, 0, 0)))
```

## 5. Write the equation of the model you fit in number 4.

$$
x_t - x_{t-1} = \alpha + \phi_1(x_{t-1} - x_{t-2}) + w_t\text{; } w_t \text{~} N(0, \theta^2_w)
$$

## 6. Provide and interpret 3-5 diagnostic plots or tests and comment on whether the model
fits well.
...
## 10. Use the automatic algorithm in the ARIMA() function to fit a model to the snowshoe
hare time series.
...
automatic model has a slightly lower AICc value (2102.85 v. 2110.88) and its
...
what I might expect the next few years to look like, showing an increase in hare 
pelts sold unlike the continued decrease my model predicts. 

# Transforming

## 1. For the following series, find an appropriate transformation and order of differencing to
