...
3.  \[5 points\] Write down $\gamma_x(s,t)$, the autocovariance function of $x_t$ \[3 points\]. You may accomplish this in any way, including asking me personally in office hours or asking a classmate. Just make sure you cite the source\![2 points\]
    ...
    $\gamma_x(s,t) = cov(x_s, x_t) = cov(2\cos(2\pi\frac{s+15}{50}) + w_s, 2\cos(2\pi\frac{t+15}{50}) + w_t)$

    Since $cov(X + a, Y + b) = cov(X, Y)$, where $X$ and $Y$ are random variables and $a$ and $b$ are constants, we can say

    $cov(2\cos(2\pi\frac{s+15}{50}) + w_s, 2\cos(2\pi\frac{t+15}{50}) + w_t) = cov(w_s, w_t) = \sigma^2_w$ if $s = t$ and $0$ if $s \ne t$
