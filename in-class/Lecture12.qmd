...
Since `sarima` outputs diagnostics when you fit the model, you could
just re-fit the model with `sarima` and then use the diagnostics.
...
How can we do this - by "hand" (or just "manually" using code) - using
`fable` or `astsa` functions
...
-   `astsa` functions
    ...
    1.  Determine model order using acf/pacf, check model fit using
        `sarima`
    2.  Use `sarima.for` to forecast (with order you chose-- re-fits
        model)
...
3.  Calculate the forecast, pushing forward your next forecasted value
...
(apparent) `fable` forecast equation:
$\hat{x_t} = 0.0054 + 0.3467\cdot x_{t-1}$

(apparent) `astsa` forecast equation:
...
$$ Test statistic used to calcualte p-values in the `sarima` output--
the $\hat{\rho}_{resid}(h)$ is the sample acf we plot using `acf` or
`acf1`.
...
We have also seen the `decompose` function, but emphasized it is an
