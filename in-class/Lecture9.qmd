...
-   autocorrelation plot - 5, 6, 7
-   lag plot - 5
-   trend estimation - 1, 5, 6, 7
-   detrending - 5
-   differencing - 5
-   writing out a mathematical model - 3, 4
-   optimizing plots (adding good axis labels, etc) - 7
-   ask a time series research question - 2, 8
...
If the amount of time it takes to go through a pattern is fixed, then it is seasonal. If it is not, then it is cyclic. This data is an example of seasonal. An example of cyclic would be the foxes v. bunny population data.
...
-   Why coerce data to stationary?

    -   stationary: dont depend on time except how far apart the time points are (lag)

    -   \^ models temporal structure in a predictable, estimable, structure
...
-   code
    ...
    -   stats::filter() for MA and Autoregressions

    -   random walk

        -   white noise –\> cusum()

        -   white noise + drift –\> cusum()

    -   signal plus noise

        -   trend + white noise

-   White noise v. stationarity

    -   white noise autocorrelation function: sigma\^2 for s = t, else 0

    -   MA: a for s = t, b for \|s - t\| = 1, ....0 \|s - t\| \> \#

        -   ACF plot show some correlation at lags, up to its \# (eg 3 point MA)
